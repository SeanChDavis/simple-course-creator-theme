...
function scct_setup() {
	...
	register_nav_menus(
		array(
			'primary-menu' => __( 'Primary', 'scct' ),
			'footer-menu' => __( 'Footer', 'scct' ),
		)
	);
...
add_action( 'after_setup_theme', 'scct_setup' );

// Set the content width in pixels
function scct_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'scct_content_width', 856 );
}
add_action( 'after_setup_theme', 'scct_content_width', 0 );

// Register sidebar area.
function scct_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'scct' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'scct' ),
...
add_action( 'widgets_init', 'scct_widgets_init' );

// Enqueue scripts and styles.
function scct_scripts() {
	wp_enqueue_style( 'scct-style', get_stylesheet_uri(), array(), THEME_VERSION );
	wp_enqueue_script( 'scct-navigation', get_template_directory_uri() . '/assets/js/scripts.min.js', array(), THEME_VERSION, true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'scct_scripts' );
 ...
 * @return void
 */
function scct_disable_classic_theme_styles() {
	wp_deregister_style( 'classic-theme-styles' );
	wp_dequeue_style( 'classic-theme-styles' );
}
add_filter( 'wp_enqueue_scripts', 'scct_disable_classic_theme_styles', 100 );
