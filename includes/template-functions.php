 ...
 * @return array
 */
function scct_body_classes( $classes ) {
	global $post;
...
add_filter( 'body_class', 'scct_body_classes' );
 ...
 * @return mixed
 */
function scct_posts_only_search( $query ) {
...
add_action( 'pre_get_posts', 'scct_posts_only_search' );


/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function scct_pingback_header() {

	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'scct_pingback_header' );
 ...
 * @return void
 */
function scct_add_excerpts_to_pages() {
	add_post_type_support( 'page', 'excerpt' );
}
add_action( 'init', 'scct_add_excerpts_to_pages' );
